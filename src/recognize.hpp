...
namespace penguin {
  ...
  public:
    ...
    Widget(const Widget& widget)
      : widget_label(widget.widget_label), x(widget.x), y(widget.y),
      _img(widget._img), _parent_widget(widget._parent_widget),
      _status(widget._status), _exception_list(widget._exception_list) {}
    Widget(Widget&& widget) noexcept
      : widget_label(std::move(widget.widget_label)), x(std::move(widget.x)),
      y(std::move(widget.y)), _img(std::move(widget._img)),
  ...
  public:
    ...
    Widget_Item(FurniFlags furni, const std::string& label,
      Widget* const parent_widget = nullptr)
      : Widget(label, parent_widget), _itemId("furni"), _confidence(1),
