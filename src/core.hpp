...
enum DirectionFlags { TOP = 0, BOTTOM = 1, LEFT = 2, RIGHT = 3 };

enum RangeFlags { BEGIN = 0, END = 1 };

enum ResizeFlags { RESIZE_W16_H16 = 0, RESIZE_W32_H8 = 1, RESIZE_W8_H32 = 2 };

enum HammingFlags { HAMMING16 = 16, HAMMING64 = 64 };
...
public:
    ...
    ResourceType &get(const std::string &key) {
        if (contains(key)) {  // use contains in C++20
...
std::list<cv::Range> separate(const cv::Mat &src_bin, DirectionFlags direc,
                              int n = 0) {
