...
public:
    ...
    Rect get_baseline_v(Mat img_bin);
    Rect get_baseline_h(Mat img_bin, Rect baseline_v);
    Rect get_baseline_h_ex(Mat img_bin, Rect baseline_v);
    bool is_result(Mat img_bin, Rect baseline_v);
    bool is_3stars(Mat img_bin, Rect baseline_v);
    void get_stage();
    void get_droptypes();
    void get_drops();
};
...
Rect Result::get_baseline_v(Mat img_bin)
{
    ...
    Rect res = Rect(column, top, 1, bottom - top);
    rectangle(img_debug, res, Scalar(0, 0, 255), 2);
...
bool Result::is_result(Mat img_bin, Rect baseline_v)
{
    ...
    Size wholesize;
    Point upperleft;
    resultimg.locateROI(wholesize, upperleft);
    Rect res = Rect(upperleft.x, upperleft.y, resultimg.cols, resultimg.rows);
    rectangle(img_debug, res, Scalar(0, 0, 255), 2);
...
bool Result::is_3stars(Mat img_bin, Rect baseline_v)
{
    ...
    Rect res = Rect(sp[0], baseline_v.y, sp[1] - sp[0], baseline_v.height / 2);
    rectangle(img_debug, res, Scalar(0, 0, 255), 2);
...
void Result::get_stage()
{
    ...
    Size wholesize;
    Point upperleft;
    stageimg.locateROI(wholesize, upperleft);
    Rect res = Rect(upperleft.x, upperleft.y, stageimg.cols, stageimg.rows);
    rectangle(img_debug, res, Scalar(0, 0, 255), 2);
    upperleft.y--;
    putText(img_debug, stage, upperleft,
        FONT_HERSHEY_PLAIN, 2, Scalar(0, 0, 255), 2);
}
...
void Result::get_droptypes()
{
    ...
    if (nodes) {
        for (int i = 0; i < nodes; i = i + 2) {
            ...
            Rect res = Rect(
                sp[i], baseline_h.y, sp[i + 1] - sp[i], 1);
            rectangle(img_debug, res, Scalar(0, 0, 255));
            putText(img_debug, droptype.substr(0, 3), Point(sp[i], baseline_h.y + 12),
...
void Result::get_drops()
{
    auto detect_item = [this](Mat& dropimg) {
        ...
        if (similarity > 0.9) {
            ...
            Rect res = Rect(upperleft_.x, upperleft_.y, dropimg.cols, dropimg.rows);
            rectangle(img_debug, res, Scalar(0, 0, 255), 2);
            upperleft_.y = upperleft_.y - 2;
            putText(img_debug, itemId, upperleft_,
                FONT_HERSHEY_PLAIN, 1, Scalar(0, 0, 255));
        }
    ...
    auto detect_quantity = [this](Mat dropimg) {
        ...
        Size wholesize;
        Point upperleft;
        qimg.locateROI(wholesize, upperleft);

        int width_qimg = sp[1] - sp[digits * 2 - 2];
        upperleft.x = upperleft.x + sp[digits * 2 - 2];
        Rect res = Rect(
            upperleft.x, upperleft.y, width_qimg, qimg.rows);
        rectangle(img_debug, res, Scalar(0, 0, 255), 1);
        upperleft.y--;
        putText(img_debug, quantity, upperleft,
        ...
        for (auto& [ktype, vrange] : droptypes.items()) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (similarity > 0.9) {
                    ...
                    Mat dropimg_bin = img_bin127(Rect(
