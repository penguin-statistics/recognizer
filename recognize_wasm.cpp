...
extern "C" {
void preload_templ(char* itemId, uint8_t* buffer)
{
    Mat img;
    Mat src = Mat(183, 183, CV_8UC4, buffer);
...
extern "C" {
void recognize(int rows, int cols, uint8_t* buffer)
{
    Mat img;
    Mat src = Mat(rows, cols, CV_8UC4, buffer);
    cvtColor(src, img, COLOR_RGBA2BGR);
    Result::analayse(img);
}
}
